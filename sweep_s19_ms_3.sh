#!/bin/bash
# SWEEP S19-MS part 3/6 — Job2 GPU0
# 2 cells: A1.s3 (anchor seed 3 at C1) + γ3 (s13-anchorA revisit)
...
run_cell() {
    ...
    if (( REMAINING < ESTIMATED_RUN_SECONDS )); then
        echo "[S19-MS-3 ${CELL}/${TOTAL}] ${TAG} — SKIPPED"
        SKIPPED+=("${TAG}"); return
    fi
    local RUN_NAME="s19-ms-${TAG}-$(date +%Y%m%d-%H%M%S)"
    local LOG="/tmp/sweep_${RUN_NAME}.log"
    echo "[S19-MS-3 ${CELL}/${TOTAL}] ${RUN_NAME} (${REMAINING}s remaining)"
    echo "  $@"
...
preflight_then_full() {
    ...
    echo "[S19-MS-3 ${CELL}/${TOTAL}] ${PRE_NAME} preflight 50M steps"
...
# ===== Cell A1.s3 — anchor seed 3 at C1 =====
run_cell "A1-s3-C1" \
    --force-scale 1.1 --joint-damping 1.5e-6 --shoulder-damping 6e-7 \
    --control-cost 0.0 --control-diff-cost 0.0 --seed 3 \
    --wandb-tags s19-ms cohort sigma-anchor C1-replicate

# ===== Cell γ3 — s13-anchorA revisit (mid damping symmetric) =====
preflight_then_full "g3-s13-anchorA" \
    --force-scale 1.1 --joint-damping 9e-7 --shoulder-damping 9e-7 \
    --control-cost 0.025 --control-diff-cost 0.025 --seed 0 \
    --wandb-tags s19-ms cohort gamma s13-revisit mid-damp-symmetric

echo "================================================================"
echo "=== S19-MS part 3/6 complete ==="
