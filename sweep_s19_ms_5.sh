#!/bin/bash
# SWEEP S19-MS part 5/6 — Job3 GPU0
# 1 cell: γ5 (s16-T1f revisit, uniform tau=40ms)
...
run_cell() {
    ...
    if (( REMAINING < ESTIMATED_RUN_SECONDS )); then
        echo "[S19-MS-5 ${CELL}/${TOTAL}] ${TAG} — SKIPPED"
        SKIPPED+=("${TAG}"); return
    fi
    local RUN_NAME="s19-ms-${TAG}-$(date +%Y%m%d-%H%M%S)"
    local LOG="/tmp/sweep_${RUN_NAME}.log"
    echo "[S19-MS-5 ${CELL}/${TOTAL}] ${RUN_NAME} (${REMAINING}s remaining)"
    echo "  $@"
...
preflight_then_full() {
    ...
    echo "[S19-MS-5 ${CELL}/${TOTAL}] ${PRE_NAME} preflight 50M steps"
...
# ===== Cell γ5 — s16-T1f revisit (uniform tau=40ms across all muscles) =====
preflight_then_full "g5-s16-T1f-tau40" \
    --force-scale 1.1 --joint-damping 1.5e-6 --shoulder-damping 6e-7 \
    --control-cost 0.025 --control-diff-cost 0.025 \
    --muscle-tau-act 0.040 \
    --seed 0 \
    --wandb-tags s19-ms cohort gamma s16-revisit tau-extended

echo "================================================================"
echo "=== S19-MS part 5/6 complete ==="
