#!/bin/bash
# SWEEP S14-MS part 2/6 — Anchor A (d9e-7 cc0.025 cdc0.025) low-t asymmetric + L0 + F1
# 8 cells (E1–E6, L0, F1) × 2 seeds = 16 runs.
...
run_s14_cell() {
    ...
    echo "----------------------------------------------------------------"
    echo "[S14-MS-2 ${CELL}/${TOTAL}] ${RUN_NAME}"
    ...
    if python train_mouse_janelia_sigmoid_moving_shoulder.py "${BASE_ARGS[@]}" \
        ...
        --wandb-tags s14-ms moving-shoulder "${ANCHOR_TAG}" "${CELL_ID}" "${RATIO_TAG}" "seed${SEED}" qzero "${ZONE_TAG}" \
        2>&1 | tee "${LOG}"; then
...
# Low-t asymmetric zone + L0 (symmetric baseline) + F1 (reverse-ratio falsifier)
for SEED in 1 2; do
    run_s14_cell "E1" 0.7 1.0 "${SEED}" "low-t-asym"
    run_s14_cell "E2" 0.7 1.3 "${SEED}" "low-t-asym"
    run_s14_cell "E3" 0.8 1.0 "${SEED}" "low-t-asym"
    run_s14_cell "E4" 0.8 1.3 "${SEED}" "low-t-asym"
    run_s14_cell "E5" 0.9 1.0 "${SEED}" "low-t-asym"
    run_s14_cell "E6" 0.9 1.3 "${SEED}" "low-t-asym"
    run_s14_cell "L0" 1.0 1.0 "${SEED}" "coupled"
    run_s14_cell "F1" 1.3 1.0 "${SEED}" "falsifier"
done

echo "================================================================"
echo "=== S14-MS part 2/6 complete ==="
