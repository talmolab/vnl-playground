#!/bin/bash
# SWEEP S19-MS part 1/6 — Job1 GPU0
# 2 cells: A1.s1 (anchor seed 1 at C1) + γ1 (s10 fs=0.5 revisit, with preflight gate)
...
run_cell() {
    ...
    if (( REMAINING < ESTIMATED_RUN_SECONDS )); then
        echo "----------------------------------------------------------------"
        echo "[S19-MS-1 ${CELL}/${TOTAL}] ${TAG} — SKIPPED (budget ${REMAINING}s < ${ESTIMATED_RUN_SECONDS}s est)"
        echo "----------------------------------------------------------------"
        SKIPPED+=("${TAG}")
        return
    fi
    ...
    echo "----------------------------------------------------------------"
    echo "[S19-MS-1 ${CELL}/${TOTAL}] ${RUN_NAME} (${REMAINING}s remaining)"
    echo "  $@"
    echo "----------------------------------------------------------------"
...
# Run a 50M-step preflight (no wandb) and check final reward >= floor before committing to full run.
preflight_then_full() {
    local TAG="$1"; shift
    local NOW=$(date +%s)
    local REMAINING=$(( BUDGET_SECONDS - (NOW - START_TIME) ))
    ...
    if (( REMAINING < ESTIMATED_RUN_SECONDS + PREFLIGHT_SECONDS )); then
        echo "----------------------------------------------------------------"
        echo "[S19-MS-1 ${CELL}/${TOTAL}] ${TAG} — SKIPPED (budget ${REMAINING}s < ${ESTIMATED_RUN_SECONDS}+${PREFLIGHT_SECONDS}s)"
        echo "----------------------------------------------------------------"
        SKIPPED+=("${TAG}")
        return
    fi
    ...
    echo "----------------------------------------------------------------"
    echo "[S19-MS-1 ${CELL}/${TOTAL}] ${PRE_NAME} preflight 50M steps"
    echo "----------------------------------------------------------------"
    if ! python train_mouse_janelia_sigmoid_moving_shoulder.py \
        ...
        echo "[PREFLIGHT-CRASHED] ${PRE_NAME} (see ${PRE_LOG})"
        PREFLIGHT_FAILED+=("${TAG}")
        return
    fi
    # Extract final eval/episode_reward from preflight log
    local FINAL_REWARD
    FINAL_REWARD=$(grep -oE "eval/episode_reward[^a-zA-Z_]*: *-?[0-9]+\.[0-9]+" "${PRE_LOG}" | tail -1 | grep -oE "\-?[0-9]+\.[0-9]+" | tail -1)
    if [ -z "${FINAL_REWARD}" ]; then
        echo "[PREFLIGHT-NOMETRIC] ${TAG} — could not parse eval/episode_reward; proceeding to full run anyway"
    else
        local PASS
        PASS=$(awk -v r="${FINAL_REWARD}" -v f="${PREFLIGHT_REWARD_FLOOR}" 'BEGIN { print (r >= f) ? 1 : 0 }')
        if [ "${PASS}" != "1" ]; then
            echo "[PREFLIGHT-FAILED] ${TAG} preflight reward ${FINAL_REWARD} < ${PREFLIGHT_REWARD_FLOOR}; skipping full run"
            PREFLIGHT_FAILED+=("${TAG}")
            return
        fi
        echo "[PREFLIGHT-OK] ${TAG} preflight reward ${FINAL_REWARD} >= ${PREFLIGHT_REWARD_FLOOR}; proceeding to full run"
    fi
    run_cell "${TAG}" "$@"
}

# ===== Cell A1.s1 — anchor seed 1 at C1 (cc=0, cdc=0) =====
# No preflight: C1 already converges from s18.
run_cell "A1-s1-C1" \
...
# ===== Cell γ1 — s10 d9em7-fs0p5 revisit under cohort + p98_per_muscle =====
preflight_then_full "g1-s10-fs0p5" \
    --force-scale 0.5 --joint-damping 9e-7 --shoulder-damping 9e-7 \
    --control-cost 0.05 --control-diff-cost 0.10 --seed 0 \
    --wandb-tags s19-ms cohort gamma s10-revisit low-fs

echo "================================================================"
echo "=== S19-MS part 1/6 complete ==="
