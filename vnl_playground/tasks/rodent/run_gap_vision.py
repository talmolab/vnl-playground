...
Usage::
    ...
    vision_env = VisionRenderWrapper(
        brax_env, env.mj_model, nworld=num_envs, **vision_config
    )
"""
...
class RunGapVision(run_gap.RunGap):
    ...
    def _configure_eye_cameras(self, eye_angle_offset: float) -> None:
        ...
        The resulting binocular overlap (for square images) is approximately:
            overlap_deg ≈ fovy - 2 x degrees(eye_angle_offset)
    ...
    def _compute_eye_indices(self) -> None:
        ...
        nv_start = self._rodent_root_dof if self._config.randomize_gaps else 0
