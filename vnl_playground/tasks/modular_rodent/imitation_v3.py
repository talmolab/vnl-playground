...
class ModularImitation_v3(modular_base.ModularRodentEnv):
    ...
    def _get_modular_proprioception(
        ...
        def body_pos(name: str) -> jp.array:
            return torso_xmat.T @ (body_world_pos(name) - torso_xpos)
        ...
        return {
            ...
            "head": {
                ...
                "linvel": torso_xmat.T @ get_sensor_data(m, data, "head/linvel"), #TODO: check
            },
        }
    ...
    def _get_reward(self, prop: dict, cur: dict, metrics: dict) -> float:
        ...
        rewards = {
            ...
            "torso": {
                "orientation_z": target["torso"]["zaxis"][2], # Dot-product
    ...
    def _add_extra_metrics(self, prop: dict, cur: dict, metrics: dict) -> None:
        ...
        metrics["hand_L"] = {
            ...
            "pos_err": jp.linalg.norm(prop["hand_L"]["hand_pos"] - target["hand_L"]["pos"]),
        }
        metrics["arm_L"] = {
        ...
        metrics["foot_L"] = {
            "pos_err": jp.linalg.norm(prop["foot_L"]["foot_pos"] - target["foot_L"]["pos"]),
        }
        metrics["leg_L"] = {
    ...
    def _interpolated_target(
        ...
        return {
            ...
            "torso": {
                "zaxis":               _world_zaxis("torso"),
                "pelvis_zaxis":        _world_zaxis("pelvis"),
