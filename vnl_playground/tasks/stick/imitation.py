...
def default_config() -> config_dict.ConfigDict:
    return config_dict.create(
        ...
        reward_terms={
            ...
            # Mesh-only term, disabled by default: ``end_eff`` already scores
            # END_EFFECTORS + LEG_JOINTS, so a non-zero weight here double-counts
            # the 24 leg segments. Matches the stick-exp6 reference run.
            "leg_joints": {"exp_scale": 0.005, "weight": 0.0},
        ...
        termination_criteria={
            # root_too_far uses xy-only distance (see _root_too_far)
...
class Imitation(stick_base.StickBugEnv):
    ...
    def __init__(
        ...
        self.compile()
        if clips is None:
            ref_path = epath.Path(self._config.reference_data_path)
            if not ref_path.exists():
                ...
                if self._config.get("auto_download", False) and which is not None:
                    stick_base.StickBugEnv.download_reference_data(
                        which=which, dest=ref_path
                    )
                else:
                    raise FileNotFoundError(
                        f"Stick reference data not found at {ref_path}. Fetch it "
                        "with `StickBugEnv.download_reference_data()` (~35 MB from "
                        "talmolab/MIMIC-MJX), set config `auto_download=True`, or "
                        "pass `clips=`. See reference_data/README.md."
                    )
