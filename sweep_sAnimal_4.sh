#!/bin/bash
# SWEEP sAnimal part 4/6 — Job2 GPU1
# 8 cells: AT009 × {F3, F4, F5, F6, F7, F8} + AT012 × {C0, F1}.
...
run_cell() {
    ...
    if (( REMAINING < ESTIMATED_RUN_SECONDS )); then
        echo "----------------------------------------------------------------"
        echo "[sAnimal-4 ${CELL}/${TOTAL}] ${TAG} — SKIPPED (budget ${REMAINING}s < ${ESTIMATED_RUN_SECONDS}s est)"
        echo "----------------------------------------------------------------"
        SKIPPED+=("${TAG}")
        return
    fi
    ...
    echo "----------------------------------------------------------------"
    echo "[sAnimal-4 ${CELL}/${TOTAL}] ${RUN_NAME} (${REMAINING}s remaining)"
    echo "  $@"
    echo "----------------------------------------------------------------"
...
echo "================================================================"
echo "=== sAnimal part 4/6 complete ==="
