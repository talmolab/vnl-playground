"""
Janelia mouse forelimb imitation with Variational Information Bottleneck.

Combines the Janelia 4-DOF arm model (12 muscles) with an intention network
architecture for SCAMPER-compatible training.
...
Checkpoint format: (normalizer_params, policy_params, value_params)
Compatible with scamper/scripts/convert_vnl_checkpoint.py
"""
...
env_name = "janelia-mouse-intention"
FINETUNE_PATH = None
...
if USE_WANDB:
    wandb_kwargs = dict(
        ...
        config=env_cfg,
        name=wandb_name,
        id=f"janelia-intention-{exp_name}",
    )
...
if __name__ == "__main__":
    print("=" * 80)
    print("Janelia Mouse Arm Imitation -- Variational Information Bottleneck")
    print("=" * 80)
    ...
    def plot_intention_diagnostics(episode_data, save_path=None):
        ...
        proprio = episode_data["proprioception"]
        ...
        for j in range(n_show):
            ax.plot(actions[:, j], linewidth=0.4, alpha=0.5, label=f"a[{j}]")
        ax.set_xlabel("env step")
        ax.set_ylabel("action")
        ax.set_title("Action magnitudes")
        ax.legend(fontsize=6, ncol=2)

        # Row 2, Col 2: Joint tracking
        ax = axes[2, 2]
        ...
        for j in range(n_joints):
            ax.plot(np.abs(joint_delta[:, j]), linewidth=0.8,
                    label=joint_labels[j] if j < len(joint_labels) else f"j{j}")
        ax.set_xlabel("env step")
