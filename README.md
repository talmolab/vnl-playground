...
`xpos`, and `xquat`) by default. `FruitflyImitation` temporarily selects its
dataset-specific adapter through `reference_data_format="fruitfly"`; this option
can be removed after the fruit-fly dataset is regenerated by `stac-mjx`.
