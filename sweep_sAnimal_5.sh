#!/bin/bash
# SWEEP sAnimal part 5/6 — Job3 GPU0
# 8 cells: AT012 × {F2, F3, F4, F5, F6, F7, F8} + AT013 × {C0}.
...
run_cell() {
    ...
    if (( REMAINING < ESTIMATED_RUN_SECONDS )); then
        echo "----------------------------------------------------------------"
        echo "[sAnimal-5 ${CELL}/${TOTAL}] ${TAG} — SKIPPED (budget ${REMAINING}s < ${ESTIMATED_RUN_SECONDS}s est)"
        echo "----------------------------------------------------------------"
        SKIPPED+=("${TAG}")
        return
    fi
    ...
    echo "----------------------------------------------------------------"
    echo "[sAnimal-5 ${CELL}/${TOTAL}] ${RUN_NAME} (${REMAINING}s remaining)"
    echo "  $@"
    echo "----------------------------------------------------------------"
...
echo "================================================================"
echo "=== sAnimal part 5/6 complete ==="
