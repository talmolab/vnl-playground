"""Excitatory ablation dose-response: graded silencing of E neurons.

Loads a trained checkpoint, silences increasing fractions of excitatory neurons
(0%, 25%, 50%, 75%, 100%), and measures how behavior degrades.

Outputs:
  - dose_response.png : reward, E/I rates, balance vs ablation %
  - population_comparison.png : per-layer E/I activity across conditions
  - rate_distributions.png : per-neuron rate shift under ablation
  - comparison.mp4 : side-by-side video (normal vs 75% ablation)

Usage:
    python ablations.py [checkpoint_path] [output_dir]
"""
...
ABLATION_FRACS = [0.0, 0.25, 0.50, 0.75, 1.0]
CMAP = plt.cm.OrRd
COLORS = [CMAP(0.15 + 0.8 * i / (len(ABLATION_FRACS) - 1))
          for i in range(len(ABLATION_FRACS))]
...
OUTPUT_DIR = sys.argv[2] if len(sys.argv) > 2 else "ablation_results"
...
def make_ablated_params(frac):
    """Silence a fraction of E neurons (same random subset across layers)."""
    if frac == 0.0:
        return policy_params
    n_ablate = round(n_exc * frac)
    rng_abl = np.random.RandomState(0)  # fixed seed for reproducibility
    ablate_idx = np.sort(rng_abl.choice(n_exc, size=n_ablate, replace=False))
    abl = unfreeze(policy_params)
    for i in range(n_layers):
        lif = abl['params'][f'lif_{i}']
        # Zero feedforward input to ablated E neurons
        lif['input_proj']['kernel'] = lif['input_proj']['kernel'].at[:, ablate_idx].set(0.0)
        lif['input_proj']['bias'] = lif['input_proj']['bias'].at[ablate_idx].set(0.0)
        # Zero ablated E neurons' E→I output
        lif['W_ei'] = lif['W_ei'].at[ablate_idx, :].set(0.0)
        # Zero I→ablated-E input
        lif['W_ie'] = lif['W_ie'].at[:, ablate_idx].set(0.0)
    return freeze(abl)
...
# =============================================================================
# Run dose-response: diagnostic rollout for each ablation fraction
# =============================================================================
...
for frac in ABLATION_FRACS:
    n_ablate = round(n_exc * frac)
    label = f"{frac:.0%}"
    print(f"\n--- E ablation = {label} ({n_ablate}/{n_exc} silenced) ---")
    params = make_ablated_params(frac)
...
fig.suptitle("Excitatory Ablation Dose-Response (% of E neurons silenced)",
             fontsize=13, fontweight='bold')

fracs = np.array(ABLATION_FRACS) * 100  # as percentages
total_rewards = np.array([results[f]['total_reward'] for f in ABLATION_FRACS])

# — Panel A: Total reward vs ablation % —
ax = axes[0]
ax.plot(fracs, total_rewards, 'o-', color='black', lw=1.5, markersize=6)
ax.axhline(total_rewards[0], color='gray', ls=':', lw=0.6, alpha=0.5)
for i, f in enumerate(ABLATION_FRACS):
    pct = (total_rewards[i] - total_rewards[0]) / max(abs(total_rewards[0]), 1e-6) * 100
    ax.annotate(f'{pct:+.1f}%', (fracs[i], total_rewards[i]),
                textcoords='offset points', xytext=(0, 10), ha='center', fontsize=7)
ax.set_xlabel('E neurons silenced (%)')
ax.set_ylabel('Total episode reward')
ax.set_title('Reward vs Ablation Level')

# — Panel B: Mean E firing rate vs ablation % —
ax = axes[1]
for li, lname in enumerate(sorted(results[0.0]['layer_stats'].keys())):
    rates = [results[f]['layer_stats'][lname]['mean_e'] for f in ABLATION_FRACS]
    ax.plot(fracs, rates, 'o-', lw=1.2, markersize=5, label=lname, color=f'C{li}')
ax.set_xlabel('E neurons silenced (%)')
ax.set_ylabel('Mean E firing rate')
ax.set_title('Excitatory Rate vs Ablation')
ax.legend(framealpha=0.8)

# — Panel C: Mean I firing rate vs ablation % —
ax = axes[2]
for li, lname in enumerate(sorted(results[0.0]['layer_stats'].keys())):
    rates = [results[f]['layer_stats'][lname]['mean_i'] for f in ABLATION_FRACS]
    ax.plot(fracs, rates, 's-', lw=1.2, markersize=5, label=lname, color=f'C{li}')
ax.set_xlabel('E neurons silenced (%)')
ax.set_ylabel('Mean I firing rate')
ax.set_title('Inhibitory Rate vs Ablation')
ax.legend(framealpha=0.8)

# — Panel D: E/I balance vs ablation % —
ax = axes[3]
for li, lname in enumerate(sorted(results[0.0]['layer_stats'].keys())):
    balance = [results[f]['layer_stats'][lname]['mean_e'] /
               max(results[f]['layer_stats'][lname]['mean_i'], 1e-8)
               for f in ABLATION_FRACS]
    ax.plot(fracs, balance, 'D-', lw=1.2, markersize=5, label=lname, color=f'C{li}')
ax.axhline(1.0, color='black', ls=':', lw=0.6, alpha=0.4)
ax.set_xlabel('E neurons silenced (%)')
ax.set_ylabel('E/I rate ratio')
ax.set_title('E/I Balance vs Ablation')
ax.legend(framealpha=0.8)
...
fig.suptitle("E/I Population Rates Across Ablation Levels",
             fontsize=13, fontweight='bold', y=0.995)

for row, lname in enumerate(sorted(results[0.0]['diag'].keys())):
    T_ep = results[0.0]['diag'][lname]['sp_e'].shape[0]
    t = np.arange(T_ep)
    sigma = max(1, T_ep // 40)

    # — Col 0: E population rate —
    ax = axes[row, 0]
    for idx, frac in enumerate(ABLATION_FRACS):
        rate = gaussian_filter1d(
            np.mean(results[frac]['diag'][lname]['sp_e'], axis=(1, 2)), sigma)
        ax.plot(t, rate, color=COLORS[idx], lw=0.9,
                label=f'{frac:.0%}', alpha=0.85)
    ax.set_xlabel('Time (env steps)')
    ax.set_ylabel('E firing rate')
    ax.set_title(f'{lname} — Excitatory Population')
    ax.legend(title='Ablated', framealpha=0.8, ncol=2, fontsize=7)
    ax.set_xlim(0, T_ep)

    # — Col 1: I population rate —
    ax = axes[row, 1]
    for idx, frac in enumerate(ABLATION_FRACS):
        rate = gaussian_filter1d(
            np.mean(results[frac]['diag'][lname]['sp_i'], axis=(1, 2)), sigma)
        ax.plot(t, rate, color=COLORS[idx], lw=0.9,
                label=f'{frac:.0%}', alpha=0.85)
    ax.set_xlabel('Time (env steps)')
    ...
    # — Col 2: Cumulative reward —
    ax = axes[row, 2]
    for idx, frac in enumerate(ABLATION_FRACS):
        cum_r = np.cumsum(results[frac]['rewards'])
        ax.plot(cum_r, color=COLORS[idx], lw=0.9, label=f'{frac:.0%}', alpha=0.85)
    ax.set_xlabel('Time (env steps)')
...
fig.suptitle("Per-Neuron Firing Rate Shift Under Excitatory Ablation",
             fontsize=13, fontweight='bold', y=0.995)

key_fracs = [0.0, 0.50, 1.0]

for row, lname in enumerate(sorted(results[0.0]['diag'].keys())):
    # E neurons
    ax = axes[row, 0]
    for frac in key_fracs:
        sd = results[frac]['diag'][lname]
        rates = np.mean(sd['sp_e'], axis=(0, 1))
        idx = ABLATION_FRACS.index(frac)
        ax.hist(rates, bins=40, alpha=0.5, color=COLORS[idx],
                label=f'{frac:.0%} ($\\mu$={np.mean(rates):.4f})', density=True)
    ax.set_xlabel('Mean firing rate')
    ax.set_ylabel('Density')
    ax.set_title(f'{lname} — E Neuron Rate Distributions')
    ax.legend(title='Ablated', framealpha=0.8, fontsize=7)
...
# =============================================================================
# Videos: Normal (0%) vs 75% ablation
# =============================================================================

VIDEO_FRAC = 0.75
print(f"\nRendering videos: 0% vs {VIDEO_FRAC:.0%} E ablation...")

normal_states, _ = video_rollout(policy_params, normalizer_params, seed=SEED)
abl_params = make_ablated_params(VIDEO_FRAC)
abl_states, _ = video_rollout(abl_params, normalizer_params, seed=SEED)

fps = int(1.0 / eval_env.dt)
normal_frames = eval_env.render(normal_states, height=512, width=512, render_ghost=True)
abl_frames = eval_env.render(abl_states, height=512, width=512, render_ghost=True)

for name, frames in [("normal_rollout.mp4", normal_frames),
                     (f"ablated_{VIDEO_FRAC:.0%}_rollout.mp4", abl_frames)]:
...
n_frames = min(len(normal_frames), len(abl_frames))
frame_h, frame_w = normal_frames[0].shape[:2]
sep_w = 6

try:
    label_l = render_text_bar("NORMAL (0%)", frame_w, 48,
                              bg_color=(44, 160, 44))
    n_abl = round(n_exc * VIDEO_FRAC)
    label_r = render_text_bar(f"{VIDEO_FRAC:.0%} E ABLATED ({n_abl}/{n_exc})",
                              frame_w, 48, bg_color=(214, 39, 40))
...
with imageio.get_writer(comp_path, fps=fps) as vid:
    for i in range(n_frames):
        combined = np.concatenate(
            [normal_frames[i], separator, abl_frames[i]], axis=1)
        if has_labels:
...
print("\n" + "=" * 70)
print("EXCITATORY ABLATION RESULTS")
print("=" * 70)
print(f"{'Ablated':>8s}  {'Reward':>10s}  {'Δ%':>8s}", end="")
for lname in sorted(results[0.0]['layer_stats'].keys()):
    print(f"  {lname} E rate  {lname} I rate", end="")
print()
print("-" * 70)
baseline_r = results[0.0]['total_reward']
for frac in ABLATION_FRACS:
    r = results[frac]
    n_abl = round(n_exc * frac)
    pct = (r['total_reward'] - baseline_r) / max(abs(baseline_r), 1e-6) * 100
    print(f" {frac:>6.0%}  {r['total_reward']:>10.2f}  {pct:>+7.1f}%", end="")
...
print("  dose_response.png          — reward & rates vs ablation level")
print("  population_comparison.png  — E/I activity traces across conditions")
print("  rate_distributions.png     — per-neuron rate shift under ablation")
print("  normal_rollout.mp4         — baseline video")
print(f"  ablated_{VIDEO_FRAC:.0%}_rollout.mp4     — ablated video")
