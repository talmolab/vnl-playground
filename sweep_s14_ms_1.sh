#!/bin/bash
# SWEEP S14-MS part 1/6 — Anchor A (d9e-7 cc0.025 cdc0.025) coupled diagonal
# 8 cells (C1–C8) × 2 seeds = 16 runs.
...
compute_override() {
    # override = 0.1 (XML default) * x_eff / fs_shoulder  ->  effective_force = 0.1 * x_eff
    python3 -c "print(0.1 * $1 / ${FS_SHOULDER})"
}

fs_str() {
    # 1.0 -> 1p0, 0.7 -> 0p7, 1.15 -> 1p15
    echo "$1" | tr '.' 'p'
}

run_s14_cell() {
    local CELL_ID="$1"    # e.g., "C1"
    local T_EFF="$2"
    local B_EFF="$3"
    local SEED="$4"
    ...
    echo "----------------------------------------------------------------"
    echo "[S14-MS-1 ${CELL}/${TOTAL}] ${RUN_NAME}"
    ...
    if python train_mouse_janelia_sigmoid_moving_shoulder.py "${BASE_ARGS[@]}" \
        ...
        --wandb-tags s14-ms moving-shoulder "${ANCHOR_TAG}" "${CELL_ID}" "${RATIO_TAG}" "seed${SEED}" qzero coupled \
        2>&1 | tee "${LOG}"; then
...
# Coupled diagonal (C1–C8): t_eff = b_eff, each seed 1 then seed 2.
for SEED in 1 2; do
    run_s14_cell "C1" 0.7 0.7 "${SEED}"
    run_s14_cell "C2" 0.8 0.8 "${SEED}"
    run_s14_cell "C3" 0.9 0.9 "${SEED}"
    run_s14_cell "C4" 1.1 1.1 "${SEED}"
    run_s14_cell "C5" 1.2 1.2 "${SEED}"
    run_s14_cell "C6" 1.3 1.3 "${SEED}"
    run_s14_cell "C7" 1.4 1.4 "${SEED}"
    run_s14_cell "C8" 1.5 1.5 "${SEED}"
done

echo "================================================================"
echo "=== S14-MS part 1/6 complete ==="
