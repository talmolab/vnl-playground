#!/bin/bash
# SWEEP sAnimal part 2/6 — Job1 GPU1
# 8 cells: A36-1 × {F5, F6, F7, F8} + AT006 × {C0, F1, F2, F3}.
...
run_cell() {
    ...
    if (( REMAINING < ESTIMATED_RUN_SECONDS )); then
        echo "----------------------------------------------------------------"
        echo "[sAnimal-2 ${CELL}/${TOTAL}] ${TAG} — SKIPPED (budget ${REMAINING}s < ${ESTIMATED_RUN_SECONDS}s est)"
        echo "----------------------------------------------------------------"
        SKIPPED+=("${TAG}")
        return
    fi
    ...
    echo "----------------------------------------------------------------"
    echo "[sAnimal-2 ${CELL}/${TOTAL}] ${RUN_NAME} (${REMAINING}s remaining)"
    echo "  $@"
    echo "----------------------------------------------------------------"
...
echo "================================================================"
echo "=== sAnimal part 2/6 complete ==="
