# Lab Meeting Animation Suite Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build `lab_meeting_2_23_26_animation.py` — a single script that loads the propriospinal-MN checkpoint, collects neural diagnostics (spikes, voltages, actions) from baseline/stimulation/ablation rollouts, and generates synchronized multi-panel MP4 videos.

**Architecture:** Modular script with three phases: (1) data collection via diagnostic rollouts, (2) panel pre-rendering with matplotlib, (3) frame-by-frame video composition with time cursors. Imports network classes from `train_mouse_propriospinal_mn.py`.

**Tech Stack:** JAX, Flax, MuJoCo (EGL), matplotlib (Agg backend), imageio, numpy, sklearn (PCA)

---

### Task 1: Scaffold — imports, args, env setup, checkpoint loading

**Files:**
- Create: `lab_meeting_2_23_26_animation.py`
...
**Step 2: Run script to verify it loads the checkpoint**

Run: `cd /root/vast/eric/vnl-playground && source /root/vast/eric/track-mjx/.venv/bin/activate && python lab_meeting_2_23_26_animation.py 2>&1 | tail -20`
Expected: "Checkpoint loaded." with no errors.

**Step 3: Commit**

```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "scaffold: lab meeting animation script with checkpoint loading"
```

---

### Task 2: Data collection — diagnostic rollout + video rollout + combined collector

**Files:**
- Modify: `lab_meeting_2_23_26_animation.py` (append after checkpoint loading)

**Step 1: Write the rollout functions**

Append the following after the JIT compile section:
...
**Step 2: Write the stimulation/ablation param manipulators**
...
**Step 3: Write the main data collection orchestrator**
...
**Step 4: Run to verify data collection**

Run: `cd /root/vast/eric/vnl-playground && source /root/vast/eric/track-mjx/.venv/bin/activate && python lab_meeting_2_23_26_animation.py 2>&1 | tail -30`
Expected: "All data collected." with reward values printed for each condition.

**Step 5: Commit**

```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "feat: data collection for baseline, stimulation, and ablation rollouts"
```

---

### Task 3: Panel rendering utilities

**Files:**
- Modify: `lab_meeting_2_23_26_animation.py` (insert between data collection and video generation)

**Step 1: Write panel rendering functions**

Insert after the data collection section:
...
def render_raster_panel(spikes, title, cmap, n_show=None, v_th=None):
    ...
    Args:
        ...
        v_th: ignored (present for API consistency)
...
Run: `cd /root/vast/eric/vnl-playground && source /root/vast/eric/track-mjx/.venv/bin/activate && python -c "
import lab_meeting_2_23_26_animation as lm
import numpy as np
# Test raster render with dummy data
spikes = np.random.binomial(1, 0.1, (80, 8, 100))
img = lm.render_raster_panel(spikes, 'Test', lm.CMAP_EXC)
print('Raster panel shape:', img.shape)
"`
Expected: `Raster panel shape: (320, 480, 3)`

**Step 3: Commit**

```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "feat: panel rendering utilities (rasters, voltages, PCA, heatmaps)"
```

---

### Task 4: Video generator — baseline_full.mp4

**Files:**
- Modify: `lab_meeting_2_23_26_animation.py` (append video generation functions)

**Step 1: Write the baseline full video generator**
...
**Step 2: Wire it into the main flow and test**
...
Run: `cd /root/vast/eric/vnl-playground && source /root/vast/eric/track-mjx/.venv/bin/activate && python lab_meeting_2_23_26_animation.py 2>&1 | tail -5`
Expected: `Saved: outputs/lab_meeting_2_23_26/baseline_full.mp4`

**Step 3: Commit**

```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "feat: baseline full video with 2x4 grid (rasters, PCA, actions, voltages)"
```

---

### Task 5: Video generator — baseline_membrane_potentials.mp4

**Files:**
- Modify: `lab_meeting_2_23_26_animation.py`

**Step 1: Write the membrane potential video generator**
...
```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "feat: membrane potential video with voltage heatmaps and drive traces"
```

---

### Task 6: Video generator — spike_diagnostics_baseline.mp4

**Files:**
- Modify: `lab_meeting_2_23_26_animation.py`

**Step 1: Write the spike diagnostics video generator**
...
```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "feat: spike diagnostics video (3x2 rasters, rates, PCA)"
```

---

### Task 7: Video generator — stimulation comparison videos

**Files:**
- Modify: `lab_meeting_2_23_26_animation.py`

**Step 1: Write the stimulation comparison video generator**
...
def make_comparison_video(data_normal, data_condition, label_normal, label_condition,
    ...
    Each side (5 rows): MuJoCo | PS-E raster | PS-I raster | MN raster | Actions
    ...
    for t in range(min(T, len(frames_l), len(frames_r))):
        ...
        # Row 1-4: rasters + actions + voltages
...
**Step 2: Generate all stimulation videos**
...
```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "feat: stimulation comparison videos (normal vs each dose)"
```

---

### Task 8: Video generator — ablation comparison videos

**Files:**
- Modify: `lab_meeting_2_23_26_animation.py`

**Step 1: Generate ablation videos (reuses make_comparison_video)**
...
```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "feat: ablation comparison videos"
```

---

### Task 9: Stimulation neural detail videos

**Files:**
- Modify: `lab_meeting_2_23_26_animation.py`

**Step 1: Write stimulation neural detail video**

For each stimulation dose, generate a video focused on how neural populations change:
...
def make_stim_neural_detail_video(data_normal, data_stim, sf, output_path):
    ...
    Plus firing rate traces and PCA below.
    """
    print(f"Generating {os.path.basename(output_path)}...")
...
```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "feat: stimulation neural detail videos showing population changes"
```

---

### Task 10: Summary printout and final polish

**Files:**
- Modify: `lab_meeting_2_23_26_animation.py`

**Step 1: Add summary at the end**
...
Run: `cd /root/vast/eric/vnl-playground && source /root/vast/eric/track-mjx/.venv/bin/activate && python lab_meeting_2_23_26_animation.py 2>&1`
Expected: All videos generated without errors, summary table printed.

**Step 3: Verify output files exist**

Run: `ls -la outputs/lab_meeting_2_23_26/*.mp4 | wc -l`
Expected: 19 videos total (3 baseline + 5 stim comparison + 5 stim neural + 4 ablation + 1 stim detail for each = check count)

Actually: 3 baseline + 5 stim + 5 stim neural detail + 4 ablation = 17 videos.

**Step 4: Final commit**

```bash
git add lab_meeting_2_23_26_animation.py
git commit -m "feat: complete lab meeting animation suite with all video types"
```
...
| File | Purpose |
|------|---------|
| `train_mouse_propriospinal_mn.py` | Network classes, `flatten_obs`, `ppo_params` — **import from here** |
| `stimulation.py` | Reference for stim param manipulation (but uses old `train_mouse_LIF` imports) |
| `ablations.py` | Reference for ablation param manipulation (same caveat) |
| `visualize_behavior_spiking.py` | Reference for multi-panel video composition + time cursor |
| `checkpoints/mouse-propriospinal-mn-20260223-202959/600023040` | Latest checkpoint to load |
| `checkpoints/mouse-propriospinal-mn-20260223-202959/config.json` | Env config |

## Reference: Param Structure for Propriospinal-MN

```
params/
  propriospinal/
    input_proj/kernel  (obs_size, 512)
    input_proj/bias    (512,)
    W_ie               (102, 410)
    W_ei               (410, 102)
    tau_m              (512,)
  ...
  W_inh_mn             (128, 102)
  muscle_readout/
    kernel             (128, param_size)
    bias               (param_size,)
```

## Reference: Diagnostic Output Shapes

Per env step (batch_size=1):
- `ps_diag['spikes_exc']`: (K=8, 1, 410)
- `ps_diag['spikes_inh']`: (K=8, 1, 102)
- `ps_diag['voltages_exc']`: (K=8, 1, 410)
- `ps_diag['voltages_inh']`: (K=8, 1, 102)
- `mn_diag['spikes']`: (K=8, 1, 128)
- `mn_diag['voltages']`: (K=8, 1, 128)
- `diagnostics['mn_input_exc']`: (1, 128)
- `diagnostics['mn_input_inh']`: (1, 128)
