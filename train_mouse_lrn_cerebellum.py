...
# =============================================================================
# Module 3: Cerebellum (Differentiable Kalman Filter)
# =============================================================================
...
class CerebellumModule(linen.Module):
    ...
    def __call__(self, sensory_obs, efference_copy, x_hat, P_diag):
        ...
        gate_log_scale = self.param(
            'gate_log_scale', lambda k, s: jp.zeros(s), (1,))
        gate = jp.tanh(innov_magnitude * jp.exp(gate_log_scale))  # ∈ [0, 1)
...
class LRNCerebellumPolicy(linen.Module):
    """Combined ProprioSpinal -> LRN -> Cerebellum policy.

    Carry layout:
      [ps_v | lrn_v | ps_r | lrn_r | cb_x_hat | cb_P_diag]
       (ps)   (lrn)   (ps)   (lrn)    (cb)       (cb)
    """
    ...
    def __call__(self, obs, carry_flat):
        ...
        # 2. Motor readout from propriospinal E population only
        ...
        w_raw = self.param('correction_weight_raw',
                           lambda k, s: jp.full(s, -5.0), (1,))
        w = jax.nn.sigmoid(w_raw)
        logits = raw_motor_cmd + w * correction

        # 6. Reassemble carry
        new_carry = jp.concatenate([
            ps_v_new, lrn_v_new, ps_r_new, lrn_r_new,
            cb_x_hat_new, cb_P_new,
        ], axis=-1)

        return logits, new_carry, sensory_pred_loss
...
class DiagnosticCerebellumModule(linen.Module):
    ...
    def __call__(self, sensory_obs, efference_copy, x_hat, P_diag):
        ...
        gate_log_scale = self.param(
            'gate_log_scale', lambda k, s: jp.zeros(s), (1,))
        gate = jp.tanh(innov_magnitude * jp.exp(gate_log_scale))
...
def plot_perturbation_comparison(clean_data, perturbed_data,
    ...
    for col, (data, label) in enumerate(
        [(clean_data, "Clean"), (perturbed_data, "Perturbed")]
    ):
        ax = axes[0, col]
        ax.plot(_get_innov(data), color='purple', linewidth=1)
        ax.set_xlabel("env step")
        ax.set_ylabel("||innovation||")
    ...
    # Row 1: correction magnitude
    for col, (data, label) in enumerate(
        [(clean_data, "Clean"), (perturbed_data, "Perturbed")]
    ):
        ax = axes[1, col]
        ax.plot(_get_corr_norm(data), color='darkorange', linewidth=1)
        ax.set_xlabel("env step")
        ax.set_ylabel("||correction||")
...
def make_training_fns(train_env, policy_module, value_module, action_dist,
    ...
    def collect_rollout(state: TrainingState):
        def step_fn(carry, _):
            ...
            logits, new_carry, _pred_loss = policy_module.apply(
    ...
    def sgd_step(carry, minibatch_idx, data, perm):
        ...
        def loss_fn(params):
            ...
            logits, _, sensory_pred_loss = policy_module.apply(
...
def make_perturbed_eval_fn(eval_env, diag_policy_module, action_dist,
                           episode_length, carry_dim, act_size,
                           start_frac, end_frac, scale):
    """Diagnostic eval with perturbation applied during a window."""
    ...
    def eval_episode_perturbed(policy_params, normalizer_params, rng):
        ...
        carry = jp.zeros((1, carry_dim))
        # Normalize direction so every eval gets consistent perturbation magnitude
        perturb_dir = jax.random.normal(force_rng, (1, act_size))
        perturb_force = (perturb_dir / (jp.linalg.norm(perturb_dir) + 1e-8)
                         ) * scale * jp.sqrt(jp.float32(act_size))
        ...
        def step_fn(carry_state, step_idx):
            ...
            action = action + perturb_force * in_window
...
ppo_params = config_dict.create(
    ...
    network_factory=config_dict.create(
        ...
        perturb_scale=0.5,          # force magnitude per actuator
...
if __name__ == "__main__":
    ...
    class PerturbationWrapper:
        """Applies sustained random force perturbation during a mid-episode window.

        On reset: samples a perturbation direction and coin-flips whether
        this episode is perturbed.  On step: adds perturbation to the action
        when inside the [start, end) window.
        """
        ...
        def reset(self, rng):
            ...
            # Normalize direction so perturbation magnitude is consistently scale * sqrt(act_size)
            force_dir = jax.random.normal(dir_rng, (self._env.action_size,))
            force_dir = force_dir / (jp.linalg.norm(force_dir) + 1e-8) * jp.sqrt(
                jp.float32(self._env.action_size))
            active = (jax.random.uniform(coin_rng) < self._prob).astype(
                jp.float32
            )
            new_info = {
                **state.info,
                'perturb_force': force_dir * self._scale,
        ...
        def step(self, state, action):
            ...
            perturbed_action = action + state.info['perturb_force'] * mask
            next_state = self._env.step(state, perturbed_action)
            new_info = {
                **next_state.info,
                'perturb_force': state.info['perturb_force'],
    ...
    def video_rollout(policy_params, normalizer_params, seed=0,
                       perturb_force=None):
        """Video rollout with optional perturbation.

        Args:
            perturb_force: if not None, (act_size,) array applied during
                           [perturb_start, perturb_end) window.
        """
        ...
        for t in range(episode_length):
            ...
            logits, new_carry, _ = jit_vid_policy(
                policy_params, obs_norm, carry
            )
            action = jp.squeeze(action_dist.mode(logits), axis=0)
            if perturb_force is not None and perturb_start <= t < perturb_end:
                action = action + perturb_force
    ...
    while state.total_steps < num_timesteps:
        ...
        if state.total_steps >= next_eval_at or state.total_steps >= num_timesteps:
            ...
            try:
                ...
                p_dir = jax.random.normal(p_rng, (act_size,))
                p_force = np.array(
                    (p_dir / (jp.linalg.norm(p_dir) + 1e-8))
                    * nf.perturb_scale * jp.sqrt(jp.float32(act_size))
                )
                pert_states = video_rollout(
                    state.policy_params, state.normalizer_params,
                    seed=state.total_steps,
                    perturb_force=p_force,
                )
