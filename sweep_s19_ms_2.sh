#!/bin/bash
# SWEEP S19-MS part 2/6 — Job1 GPU1
# 2 cells: A1.s2 (anchor seed 2 at C1) + γ2 (s11-goldi revisit, with preflight gate)
...
run_cell() {
    ...
    if (( REMAINING < ESTIMATED_RUN_SECONDS )); then
        echo "[S19-MS-2 ${CELL}/${TOTAL}] ${TAG} — SKIPPED (budget ${REMAINING}s < ${ESTIMATED_RUN_SECONDS}s est)"
        SKIPPED+=("${TAG}")
        return
    fi
    local RUN_NAME="s19-ms-${TAG}-$(date +%Y%m%d-%H%M%S)"
    local LOG="/tmp/sweep_${RUN_NAME}.log"
    echo "[S19-MS-2 ${CELL}/${TOTAL}] ${RUN_NAME} (${REMAINING}s remaining)"
    echo "  $@"
...
preflight_then_full() {
    ...
    if (( REMAINING < ESTIMATED_RUN_SECONDS + PREFLIGHT_SECONDS )); then
        echo "[S19-MS-2 ${CELL}/${TOTAL}] ${TAG} — SKIPPED (budget ${REMAINING}s < ${ESTIMATED_RUN_SECONDS}+${PREFLIGHT_SECONDS}s)"
        SKIPPED+=("${TAG}")
        return
    fi
    local PRE_NAME="s19-ms-${TAG}-preflight-$(date +%Y%m%d-%H%M%S)"
    local PRE_LOG="/tmp/sweep_${PRE_NAME}.log"
    echo "[S19-MS-2 ${CELL}/${TOTAL}] ${PRE_NAME} preflight 50M steps"
    ...
    if [ -z "${FINAL_REWARD}" ]; then
        echo "[PREFLIGHT-NOMETRIC] ${TAG} — proceeding anyway"
    else
        local PASS
...
# ===== Cell A1.s2 — anchor seed 2 at C1 =====
run_cell "A1-s2-C1" \
    --force-scale 1.1 --joint-damping 1.5e-6 --shoulder-damping 6e-7 \
    --control-cost 0.0 --control-diff-cost 0.0 --seed 2 \
    --wandb-tags s19-ms cohort sigma-anchor C1-replicate

# ===== Cell γ2 — s11-goldi revisit (low damping symmetric) =====
preflight_then_full "g2-s11-goldi" \
    --force-scale 1.0 --joint-damping 5e-7 --shoulder-damping 5e-7 \
    --control-cost 0.05 --control-diff-cost 0.10 --seed 0 \
    --wandb-tags s19-ms cohort gamma s11-revisit low-damp

echo "================================================================"
echo "=== S19-MS part 2/6 complete ==="
