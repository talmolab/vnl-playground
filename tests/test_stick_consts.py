...
def test_stick_box_xml_is_shipped_but_not_env_loadable():
    """The legacy box XML still ships, but no stick env can load it.

    `consts.FOOT_GEOMS`/`JOINTS`/`BODIES` all describe the mesh walker and
    `base.add_stick` resolves FOOT_GEOMS unconditionally, so the box model is
    file-present only. This test pins that limitation so the "box model is
    still available" reading cannot be mistaken for "still usable".
    """
    import pytest

    from vnl_playground.tasks.stick import consts
    from vnl_playground.tasks.stick.maintain_velocity import (
        MaintainVelocity,
        default_config,
    )
    ...
    cfg = default_config()
    cfg.walker_xml_path = consts.STICK_BOX_XML_PATH
    with pytest.raises(ValueError, match="claw_collide"):
        MaintainVelocity(config=cfg)
