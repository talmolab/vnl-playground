...
def test_eye_joints_exist_in_model():
    ...
    left_id = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_JOINT, f"eye_left_yaw{suffix}")
    right_id = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_JOINT, f"eye_right_yaw{suffix}")
...
def test_eye_actuators_exist_in_model():
    ...
    assert right_id == m.nu - 1, (
        f"Right eye actuator at {right_id}, expected {m.nu - 1}"
    )
