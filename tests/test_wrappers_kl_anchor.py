...
def test_wrapper_stores_pretanh_distribution_params_and_passes_through_r_task():
    ...
    def fake_prior_fn(proprio):
        ...
        return jnp.full((latent_size,), 0.123, dtype=jnp.float32), jnp.zeros(
            (latent_size,)
        )
    ...
    np.testing.assert_allclose(
        np.asarray(state0.info["anchor_mu_imit"]),
        np.full((action_size,), 0.5),
        atol=1e-6,
    )
    ...
    np.testing.assert_allclose(
        ...
        np.full((action_size,), expected_log_std),
        atol=1e-5,
    )
    ...
    np.testing.assert_allclose(float(state1.metrics["anchor/r_task"]), 0.7, atol=1e-6)
    # anchor/r_anchor in metrics — should be 1.0 (action == mu_imit, MSE=0).
    np.testing.assert_allclose(float(state1.metrics["anchor/r_anchor"]), 1.0, atol=1e-5)
